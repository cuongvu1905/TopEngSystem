...
async function seed() {
  ...
  const projects = [
    ...
    ['proj-ai-agent', 'Nền tảng Trợ lý ảo AI Copilot Doanh nghiệp', 'Hệ thống AI Agent tự động hóa tổng hợp báo cáo, hỗ trợ viết code, phân tích dữ liệu và tích hợp Cyber API Gateway.', 'AI', 'usr-leader', 'cust-fpt'],
  ...
  const issues = [
    ...
    [8, 'AI-101', 'proj-ai-agent', 'Tích hợp Cyber API Gateway đa mô hình', 'Kết nối gateway API OpenAI & Anthropic qua Cyber server để tối ưu chi phí và tốc độ token.', 'STORY', 'DONE', 'CRITICAL', 'usr-leader', 'usr-staff'],
  ...
  await conn.query(`
    ...
    (1, 'usr-des1', 'Em đã bàn giao file Figma Design System đầy đủ cả Light và Cyber Theme rồi ạ.'),
    ...
    (8, 'usr-leader', 'Tốc độ phản hồi qua Cyber API Gateway rất nhanh, latency giảm hơn 40%.');
  `);
  ...
  await conn.query(`
    ...
    ('task-3', 'Thiết kế Cyber Cyber Obsidian Theme', 1),
  ...
  const dailyReports = [
    ...
    ['usr-staff', 'proj-ai-agent', 'Hôm nay: Kết nối thành công Cyber API Gateway, thời gian phản hồi stream đạt mức 45ms. Ngày mai: Triển khai tiếp RAG vector database. Vướng mắc: Không có.', 'Approved', 'Tuyệt vời!'],
    ...
    ['usr-des1', 'proj-crm', 'Hôm nay: Bàn giao toàn bộ Figma Components cho cả 3 theme (Light, Dark, Cyber Cyber). Hỗ trợ team dev tích hợp CSS tokens. Ngày mai: Thiết kế màn hình Kanban Board mới.', 'Approved', 'Design rất đẹp và sắc nét!'],
    ...
    ['usr-mkt1', 'proj-crm', 'Hôm nay: Viết bài hướng dẫn sử dụng tính năng mới của CRM, quay video demo tính năng Cyber theme chia sẻ lên kênh truyền thông nội bộ.', 'Approved', 'Video rất sinh động.']
  ];
  ...
  const notifications = [
    ...
    ['usr-staff', 'Hệ thống cập nhật', 'Giao diện Cyber Cyber Obsidian đã được cập nhật với hiệu ứng viền phát sáng mới.', '#dashboard', 0],
  ...
  const activityLogs = [
    ...
    ['usr-leader', 'CREATE', 'ISSUE', 'Tạo mới Issue AI-101: Tích hợp Cyber API Gateway đa mô hình.'],
    ...
    ['usr-leader', 'CHANGE_THEME', 'SYSTEM', 'Chuyển đổi giao diện hệ thống sang theme Cyber (Cyber Obsidian).']
  ];
  ...
  await conn.query(`
    ...
    ('room-ai', 'project', '⚡ Nhóm Dự án AI Copilot & Cyber', 'proj-ai-agent'),
  ...
  const messages = [
    ...
    ['msg-3', 'room-dev', 'usr-leader', 'Theme Cyber Cyber Obsidian đã lên sóng, mọi người test thử hiệu ứng glow xem nhé.'],
    ...
    ['msg-7', 'room-ai', 'usr-staff', 'API Gateway của Cyber xử lý SSE streaming cực nhanh, prompt dài 100k tokens chạy ngon lành.'],
